...
##==============================================================================
store_info <- store_info %>% 
  mutate(admi_cd = ifelse(admi_cd %in% "43785250", "43745250", admi_cd)) %>% 
  mutate(admi_cd = ifelse(admi_cd %in% "43785310", "43745310", admi_cd)) 
...
##==============================================================================
store_info <- store_info %>% 
  mutate(admi_nm = ifelse(admi_cd %in% c("41480380", "41480400"), 
                          "장단면", admi_nm)) %>% 
  mutate(admi_cd = ifelse(admi_cd %in% c("41480380", "41480400"), 
                          "41480390", admi_cd)) 
...
mega <- mega %>% 
  left_join(
    store_info %>% 
      ...
      #              store_cnt_food = 음식,
      #              store_cnt_edu = `학문/교육`),      
      rename(store_cnt_retail = 소매,
             store_cnt_acomodt = 숙박,
             store_cnt_food = 음식,
             store_cnt_estate = 부동산,
             store_cnt_tech = `과학·기술`,
             store_cnt_lease = `시설관리·임대`,
             store_cnt_edu = 교육,
             store_cnt_medical = 보건의료,
             store_cnt_leisure = `예술·스포츠`,
             store_cnt_service = `수리·개인`
             ),
    by = c("mega_cd")       
  ) %>% 
  select(base_ym:kmedicine_clinic_cnt, store_cnt_retail, store_cnt_acomodt, 
         store_cnt_food, store_cnt_estate, store_cnt_tech, store_cnt_lease, 
         store_cnt_edu, store_cnt_medical, store_cnt_leisure, store_cnt_service,
         geometry) 
...
cty <- cty %>% 
  ...
  select(base_ym:pubhealth_center_cnt, pubhealth_branch_cnt, pubhealth_clinic_cnt,
         ...
         store_cnt_retail, store_cnt_acomodt, store_cnt_food, store_cnt_estate, 
         store_cnt_tech, store_cnt_lease, store_cnt_edu, store_cnt_medical, 
         store_cnt_leisure, store_cnt_service, geometry) %>% 
